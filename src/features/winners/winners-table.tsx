...
function WinnersTableBody({ items, rowOffset }: { items: WinnerRow[]; rowOffset: number }) {
  return (
    <tbody>
      {items.map((row, i) => (
        <tr key={row.id} className={styles.tr}>
          ...
          <td className={styles.td}>
            <CarIcon color={row.color} width={60} />
          </td>
  ...
  return (
    <div>
      <table className={styles.table}>
        ...
        <WinnersTableBody items={items} rowOffset={(page - 1) * WINNERS_PER_PAGE} />
      </table>
